...
public class Java6ProviderTest {
    ...
    @Test
    public void testProvideDAOsImports() throws IOException {
        MockFileWriter fileWriter = new MockFileWriter();
        Java6Provider.provideDAOsImports(fileWriter,"UneEntitee");
        Assertions.assertEquals("import java.sql.Connection;\n" +
        "import java.sql.PreparedStatement;\n" +
        "import java.sql.ResultSet;\n" +
        "import java.sql.SQLException;\n" +
        "import java.util.ArrayList;\n\n" +
        "import pojos.UneEntitee;\n\n",fileWriter.getWrittenData());
    }

    @Test
    public void testProvideClassDeclaration() throws IOException {
        MockFileWriter fileWriter = new MockFileWriter();
        Java6Provider.provideClassDeclaration(fileWriter,"UneClasse");
        Assertions.assertEquals("public class UneClasse {\n\n",
                fileWriter.getWrittenData());
    }
    ...
    public void testCaracterization2() throws IOException {
        String expected
                ...
                + "            try {\n"
                + "                PreparedStatement query = connection.prepareStatement(\n"
                + "                    \"insert into DOG(DOG_ID, DEATH, LEGS, NAME, AGE) values(?, ?, ?, ?, ?)\");\n"
                + "                query.setInt(1, dog.getId().intValue());\n"
                + "                query.setDate(2, new java.sql.Date(dog.getDeath().getTime()));\n"
                + "                query.setInt(3, dog.getLegs());\n"
                + "                query.setString(4, dog.getName());\n"
                + "                query.setDouble(5, dog.getAge());\n"
                + "\n"
                + "                query.executeUpdate();\n"
                + "            } catch (SQLException e) {\n"
                + "                throw new DAOException(\"Unable to perform insert on database.\", e);\n"
                ...
                + "            try {\n"
                + "                PreparedStatement query = connection.prepareStatement(\n"
                + "                                \"update DOG set DEATH=?, LEGS=?, NAME=?, AGE=? where DOG_ID=?\");\n"
                + "                query.setDate(1, new java.sql.Date(dog.getDeath().getTime()));\n"
                + "                query.setInt(2, dog.getLegs());\n"
